...
class Compare(object):
        """Pipeline to compare a list of datasets.
        ...
        custom_pgen_model: string
            location of custom pgen model
        
        vj: bool
            VJ model, without D genes.
    ...
    def __init__(self, data=[],chain_type='human_T_beta',custom_pgen_model=None,gen_seqs=None,vj=False):
        ...
        if self.custom_pgen_model is None:
            if gen_seqs is None: gen=sample_olga(int(1e6),chain_type=self.chain_type,vj=self.vj)
            self.processor=Processing(chain_type=self.chain_type)
        else:
            if gen_seqs is None: gen=sample_olga(int(1e6),custom_model_folder=self.custom_pgen_model,vj=self.vj)
    ...
    def infer_models(self):
        ...
        for d in self.datasets[1:]:
            if self.custom_pgen_model is None:
                qm=SoniaLeftposRightpos(data_seqs=d,gen_seqs=self.datasets[0],chain_type=self.chain_type,vj=self.vj)
            else:
                qm=SoniaLeftposRightpos(data_seqs=d,gen_seqs=self.datasets[0],custom_pgen_model=self.custom_pgen_model,vj=self.vj)
